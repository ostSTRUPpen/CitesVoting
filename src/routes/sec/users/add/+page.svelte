<script lang="ts">
	...
	import { addUser } from '../usersFunctions';
	...
	async function handleSubmit() {
		...
		let submitUsername = username,
			...
			// !TODO! hashování hesel! (client side nebo backend... kdo ví xdd)
			submitRole = role['text'];
		const response: any = await addUser(submitUsername, submitPassword, submitRole);
		if (response['success'] === true) {
			submitUsername = password = '';
			submitPassword = username = '';
			displayDialog('Uživatel vytvořen!', 'created');
		} else {
			disableInput = false;
...
<form on:submit|preventDefault={handleSubmit}>
	...
	<input type="submit" disabled={!canSubmit} value="Vytvořit nového uživatele" />
</form>
